...
- 网络
    ...
    - http和https
        ...
        - https
            ...
            - 特点
                ...
                > 混合加密:结合非对称加密和对称加密技术。客户端使用对称加密生成密钥对传输数据进行加密，然后使用非对称加密的公钥再对秘钥进行加密，所以网络上传输的数据是**被秘钥加密的密文**和用**公钥加密后的秘密秘钥**，因此即使被黑客截取，由于没有私钥，无法获取到加密明文的秘钥，便无法获取到明文数据。
    ...
    - TCP
        ...
        - TCP粘包
            ...
            - 什么叫粘
                - 由于tcp是面向流的协议，不会按照应用开发者的期望保持send输入数据的边界，导致**接收**侧有可能一下子收到多个应用层报文，需要应用开发者自己分开
                - 由于Nagle算法（或者TCP_CORK）的存在，在**发送**的时候，就把应用开发者多次send的数据，“粘”在一个tcp报文里面发出去了，于是，先被send的数据可能需要等待一段时间，才能跟后面被send的数据一起组成报文发出去
            - 怎么做
                ...
                - 是否禁止Nagle算法?其实99%的情况下禁不禁止都一样，延迟根本不是Nagle算法导致的；就算真有问题，最优解决方案也不是屏蔽Nagle算法
...
- 计算机基础
    ...
    - cpu调度算法
        - 
    - 进程间通信方式, 线程间通信方式,互斥量，信号量

    - 进程、线程、多线程、协程
        - 推荐文章:记得回来仔细再看一遍 https://www.cnblogs.com/anding/p/5301754.html
        - 进程是什么？
            - 进程是具有一定独立功能的程序关于某个数据集合上的一次运行活动,是系统进行资源分配和调度的一个独立单位.(包括程序段,相关数据段和进程控制端PCB)
