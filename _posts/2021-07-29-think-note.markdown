...
- 语言基础
    ...
    - lua
        ...
        - lua和c#之间是怎么交互的？
            - https://blog.csdn.net/j756915370/article/details/105779176
        - 使用lua需要注意什么问题
        - lua代码的性能瓶颈定位与优化
            - CPU
...
- 计算机基础
    ...
    - 多线程
    - 代码调用栈原理
        - 操作系统会给每个线程创建一个栈用来存储函数调用时各个函数的参数、返回地址和临时变量等。
    - 打开一个网址发生了什么
...
- 网络协议
    - 比较TCP和UDP，为什么TCP要三次握手及为什么四次挥手，如何实现可靠有序UDP
    - TCP可靠协议
    - 拥塞控制
    - TCP滑动窗口
    - TCP流式传输
- 算法
    - 两个栈实现一个队列
        ...
        - 思考 两个队列实现一个栈
            - 队列a、队列b为两个空队列
            - 入栈的时候正常依次进入a队列。 
            - 出栈的时候，把a队列除了最后一个依次出队列，并加入b队列，然后a队列出栈。
    - 第K大的数
        - 暴力法。先将数组排序（nlogn），如果是从大到小，返回第K个，如果从小到大，返回第n-k个（n表示元素个数）
        - 优先队列priorityQueue，内部实现是维护一个大小为k的最小堆。
        - 快速选择算法（快排）
    - 排序
        -  超详细十大经典排序算法总结（堆排、快排比较重要）https://blog.csdn.net/weixin_41190227/article/details/86600821
        - 快速排序、二分
    - 数组和链表复杂度分析
    - 优先队列
    - 双向链表
