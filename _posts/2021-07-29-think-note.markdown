...
- 计算机基础
    ...
    - cache的作用；怎么在代码级利用局部性；高速缓存是最快的存储器吗；内存分配机制，碎片是什么；虚拟内存是什么，虚拟内存是为了解决内存不够用的问题吗；线程的堆栈是独立的吗，一个栈就够了吗
    - 调试的时候打断点的原理
    ...
    - 进程、线程、多线程、协程
        ...
        - 进程是什么？
            - 进程是操作系统分配和使用系统资源的基本单位。Windows系统利用进程把工作划分为多个独立的区域，每个应用程序实例对应一个进程。
        - 线程是什么？
            - 进程的一个实体（线程本质上是进程中一段并发运行的代码）
            - 同一进程的所有线程共享相同的资源和内存（共享代码，全局变量，环境字符串等），使得线程间上下文切换更快、可以在同一地址空间内访问内存
        - 进程、线程关系
