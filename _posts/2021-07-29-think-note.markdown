    ...
    - TCP
        ...
        - 三次握手 四次挥手
            ...
            - 三次握手过程
                ...
                - Server收到数据包后由标志位SYN=1知道Client请求建立连接，Server将标志位SYN和ACK报文都置为1，**ack (number )=J+1**，**随机产生一个值seq=K**，并将该数据包发送给Client以确认连接请求，Server进入**SYN_RCVD*状态
