...
- 计算机基础
    ...
    - 进程间通信方式, 线程间通信方式
        - 进程间通信方式
            - 信号、信号量、消息队列、共享内存，这四种使用的方法基本相同。信号和信号量不同，都可以实现同步和互斥，前者是使用信号处理器来进行的，后者是使用P、V操作来实现。
        - windows编程中互斥器、临界区区别
            - 互斥器用于进程之间互斥，临界区用户线程之间互斥
...
- C#
    ...
    - const、define的区别
        - const定义常量只是给出对应的内存地址，#define定义的常量在内存中有若干个复制品
    - 协变、逆变
    - 重载、重写
        - 重写
