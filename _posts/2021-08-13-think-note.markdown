...
- 网络
    ...
    - KCP
        - 具体可以看:KCP与网络编程模型 分享课程
        - 是什么?
            - 一句话:快速且可靠的ARQ协议
            - 本身是纯算法的实现,不负责网络层收发包,底层可以选择udp完成网络层协议的收发包,通过KCP保障包收到的可靠性
        - 特效
            - 可靠性
                - 和TCP一样,确认和重传机制
                - 每个数据包都得等待对端确认后才能从发送队列中安全的移除出去,如果超时或者丢失按照一定规则重发
            - 流量控制
                -发送方和接收方 都有一个有限大小的发送窗口和接收窗口,控制发送的频率
            - 拥塞控制
                - 识别到网络可能拥堵,减少或停止发送,等到畅通再慢慢恢复
            - 慢启动
                - 拥塞窗口减少到1之后,网络恢复后,慢慢增大发送,避免一下子加大发送,又变回拥堵(慢慢试探网络是否变好的过程)
        - 三次握手
            - 根TCP三次握手差不多
            - client 发送syn1包到server,server收到进行简单加密方案处理后发送syn2,client收到进行处理发送syn3给server,服务器对syn3 进行验证,验证没问题,代表建立连接,后下发SYNF,客户端也做建立连接标志
        - KeepAlive 
            - TCP 本身支持,UDP 不支持
            - 在client和server两端 维护 LastSendTime,LastRecvTime, 每次发包和收包进行更新
            - 更新的时候判断是否时间需要发心跳包
            - 超时:超过一定时间没收到任何包或心跳包
        - KCP 为什么比TCP快
            - RTO(超时重传的时间)计算方法
                - TCP 2倍率增速 第三次丢包的重传时间是2*2*2 = 8*RTO
                - KCP 1.5倍率增速 第三次丢包的重传时间是1.5*1.5*1.5 = 3.375*RTO
            - 快速重传
                - KCP
                    - 确认机制:UNA(此编号前所有包已收到)+ACK(该编号包已收到)
                    - 例子:接收端发了una = 2,ack = 3 给发送端,代表:2号包丢掉了
                    - 如果连续2(可以通过配置修改)次确认某个包丢失后,就可以发起重传
                - TCP
                    - 3次确认包丢失才可以重传
            - 非退让流控
                - KCP正常模式同TCP一样使用公平退让法则，即发送窗口大小由：发送缓存大小、接收端剩余接收缓存大小、丢包退让、慢启动这四要素决定。但传送及时性要求很高的小数据时，可选择仅用**前两项**来控制发送频率。以牺牲部分公平性及带宽利用率之代价，换取了流畅传输的效果。
                - KCP解决思路是降低丢包带来的影响,策略是**提高发包的频率**
                - 优点: 实时性好
                - 缺点: 但带宽利用率较低，因为非退让流控，不断尝试发送数据，有效包不多每个包应答，占用一定的带宽
            - KCP作者原话
                - TCP是为流量设计的,目标是每秒内可以传输多少KB的有效数据,讲究的是充分利用贷款
                - KCP是为流速设计的,目标是减少单个数据包从一端发到另一端所需的时间,KCP以10%~20%带宽浪费换取了比TCP快30%~40%的传输速度
        - KCP 优化思路(附带一个冗余包)
            - 冗余发包,极端情况:把所有未收到的包都发送
                - 每次都把未收到应答的包也发送, 每次都要把未确认包copy放到包尾巴
                - 缺点:流量 变多, 包copy等行为带来的cpu开销
            - 冗余策略调整
                - 冗余个数限制
                    - 从冗余候选队列拿相应个数的冗余包加入该包末尾一起发出去
                - 动态冗余
                    - 统计丢包率和RTT(一个报文段发送出去，到收到对应确认包的时间差),评估网络质量,来决定是否开启冗余
            - 在moba这种强竞技类型+对弱网表现又强要求 可以尝试 这个思路,并调整冗余配置策略,fp在研moba方案是:带一个冗余,但是没开启动态冗余
        - 其他
            - 有些运营商对UDP支持不是很友好
                - 处理方式:KCP/UDP一段时间连不上 切回TCP通信(保底机制)
    - socket 套接字
        - what? 
        ...
        - 解决多连接并发
            - 多线程的socket服务
            - IO 多路复用模型,处理一个线程处理多个socket的问题
                - 常用模型: select/poll/**epoll(linux用的比较多)**/**kqueue(unix macOS用的比较多)**
        - 阻塞模式 非阻塞模式
            - 概念
...
**Composites（复合类）**：主要用于控制行为树的走向，也是用的最多最重要的一类，任何一个相对复杂的行为树都包含这类Task节点，但它本身不做任何具体行为，所以它们一般位于父节点或根节点。
    ...
    - Draw Call 和 Set Pass Call
        - 目的
            - CPU和GPU既可以并行又可以独立哦工作，要使用一个**命令缓冲区**，命令缓冲区包含一个命令队列，当CPU需要渲染一些对象时，它会通过图像编程接口向命令缓冲区添加命令，当GPU完成上一次渲染任务后，会从命令队列中读取一个命令执行，添加和读取时相互独立的。
        - 命令缓冲区的命令有很多种，DrawCall时其中一种，其他命令有Set Pass Call等。
        - Set Pass Call：代表了**改变渲染状态**,当切换材质或者切换同一材质中shader的不同pass进行渲染时都会触发一次Set Pass Call。如，渲染1000个相同的物体和1000个不同的物体，虽然两者drawcall 都是1000，但是前者set pass call 为1， 后者为1000， **切换渲染状态往往比drawcall 更耗时**
        - 为什么要降低Draw Call
            - 调用DrawCall 之前，CPU都要向GPU发送很多内容，包括数据、状态、命令等，发送之前还需要做很多检查工作。
            - GPU的渲染能力很强，渲染速度往往比CPU的提交命令速度块，如果DrawCall过多，CPU就会把大量时间花费在提交drawCall上，造成CPU过载，帧率下降，所以需要**批处理**来降低drawcall
            - 批处理
                - 早期只支持动态批处理、静态批处理，后来支持GPU Instancing，最后SRP支持SRP Batching
...
- 图形学
    ...
    - Gamma颜色空间、线性颜色空间
        - 人眼对于光轻度的感知是**非线性**的
        - 业界约定Gamma = 2.2，因为人眼的感知能力和 n = 1/2.2的幂函数比较靠近
        - 例子
            - 举一个简单例子，两个光子投射到相机上，其获得的光强就是2倍光子光强，固然咱们已经知道人眼并非2倍光强。
            - 而业界的图片都是Gamma空间中存储（业界标准称为**sRGB**），那么相机到最终图片，就会经历一个编码过程，这就是所谓的Gamma编码，也就是: Vout = Vin(1/2.2) 这个过程。
        - 总结
            - **unity中的gamma空间和线性空间，其实质就是对存储sRGB格式图片，进行不一样的光照计算，不一样的光照计算进行不一样的流程，获得精度不一样的结果，最后都须要统一为gamma空间的格式，提交到显示器上进行gamma补偿，获得最终的显示图片。**
