...
- ILRuntime
    ...
    - 基本原理
        - 官方：ILRuntime借助Mono.Cecil库来读取DLL的PE信息，以及当中类型的所有信息，最终得到方法的IL汇编码，然后通过内置的IL解译执行虚拟机来执行DLL中的代码
        - 这段话解释就是，ILRuntime目标是读取热更新的dll，编译成IL，然后使用自己的JIT Compiler来执行热更dll中的代码,达到热更的目的。既然自己要解释执行，所以对于C#语言的基本类型就要自己进行去定义和处理，自己维护一个调用栈在其虚拟机中
        - 通过自己实现一套dll的加载和执行工具(解释执行)就可以实现最基础的动态加载能力。
        - C#运行的是IL代码，也就是中间语言。因此他有自己的套指令集。因此我们只要模仿他指令集的实现一套功能函数其实就能够实现简单模拟
    - 热更流程
    - 性能优化建议
    - 商业项目中得ILRuntime解决方案